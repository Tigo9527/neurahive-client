...
import (
	...
	"github.com/Conflux-Chain/neurahive-client/common/parallel"
...
func (downloader *SegmentDownloader) ParallelDo(routine, task int) (interface{}, error) {
	...
	if downloader.withProof {
		...
		segment, err = downloader.clients[routine].Neurahive().DownloadSegment(root, startIndex, endIndex)
	}
...
func (downloader *SegmentDownloader) downloadWithProof(client *node.Client, root common.Hash, startIndex, endIndex uint64) ([]byte, error) {
	...
	segment, err := client.Neurahive().DownloadSegmentWithProof(root, segmentIndex)
	if err != nil {
